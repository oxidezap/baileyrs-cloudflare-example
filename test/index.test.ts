...
it('continues explicit QR pairing when a concurrent status recovery finds no saved session', async () => {
  let releaseRecovery!: () => void
  const recoveryGate = new Promise<void>(resolve => { releaseRecovery = resolve })
  host.authenticate.mockImplementationOnce(async () => {
    await recoveryGate
    return { creds: { registered: false } }
  })
  const bot = new Bot(createContext(createStorage()), createEnv())

  const recovering = bot.fetch(request('/status'))
  const pairing = bot.fetch(request('/start', 'POST'))
  releaseRecovery()
  await recovering
  await pairing

  expect(host.makeSocket).toHaveBeenCalledOnce()
  await latestConnectionUpdate()({ qr: 'first-pairing-qr' })
  expect(await (await bot.fetch(request('/status'))).json()).toEqual({ state: 'waiting_for_qr', qr: 'first-pairing-qr' })
})
